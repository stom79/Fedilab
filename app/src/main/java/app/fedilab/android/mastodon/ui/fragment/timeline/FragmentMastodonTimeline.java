...
public class FragmentMastodonTimeline extends Fragment implements StatusAdapter.FetchMoreCallBack {
    ...
    private void restoreFromCache() {
        ...
        timelinesVM.getTimelineCache(timelineStatuses, timelineParams)
                .observe(getViewLifecycleOwner(), statusesCached -> {
                    ...
                    logTimelineLoad(null, TimelineLoadLogs.SOURCE_CACHE, timelineParams, statusesCached, false, displayedBefore);
                });
    }
    ...
    private void initializeAfterBundle(Bundle bundle) {
        ...
        misskeyTimelinesVM = new ViewModelProvider(FragmentMastodonTimeline.this).get(viewModelKey, MisskeyTimelinesVM.class);
        ...
        if (timelineType == Timeline.TimeLineEnum.BOOKMARK_TIMELINE || timelineType == Timeline.TimeLineEnum.FAVOURITE_TIMELINE) {
            ...
            if (requireActivity() instanceof BaseMainActivity) {
                ViewGroup.MarginLayoutParams params = (ViewGroup.MarginLayoutParams) binding.addAction.getLayoutParams();
                TypedValue tv = new TypedValue();
                if (requireActivity().getTheme().resolveAttribute(android.R.attr.actionBarSize, tv, true)) {
                    int bottomBarHeight = TypedValue.complexToDimensionPixelSize(tv.data, getResources().getDisplayMetrics());
                    params.bottomMargin += bottomBarHeight;
                    binding.addAction.setLayoutParams(params);
                }
            }
        }
        if (needToCallResume) {
            ...
            binding.recyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
                ...
                public void onScrolled(@NonNull RecyclerView recyclerView, int dx, int dy) {
                    ...
                    if (timelineType == Timeline.TimeLineEnum.BOOKMARK_TIMELINE
                            || timelineType == Timeline.TimeLineEnum.FAVOURITE_TIMELINE) {
                        if (dy > 0 && binding.addAction.isShown()) {
                            binding.addAction.hide();
                        } else if (dy < 0 && !binding.addAction.isShown()) {
                            binding.addAction.show();
                        }
                    }
    ...
    private void routeCommon(DIRECTION direction, boolean fetchingMissing, Status fetchStatus) {
        ...
        if (currentApi == app.fedilab.android.mastodon.client.entities.app.Account.API.MISSKEY) {
            routeMisskey(direction, fetchingMissing, fetchStatus);
            return;
        }
    ...
    private void routeMisskey(DIRECTION direction, boolean fetchingMissing, Status fetchStatus) {
        String maxId = null;
        String sinceId = null;
        if (direction == DIRECTION.REFRESH || direction == DIRECTION.SCROLL_TOP || direction == DIRECTION.FETCH_NEW) {
            maxId = null;
            ...
            maxId = fetchingMissing ? max_id_fetch_more : max_id;
        } else if (direction == DIRECTION.TOP) {
            sinceId = fetchingMissing ? min_id_fetch_more : min_id;
        } else {
            maxId = max_id;
        }

        int limit = MastodonHelper.statusesPerCall(requireActivity());
        String instance = BaseMainActivity.currentInstance;
        String token = BaseMainActivity.currentToken;

        LiveData<Statuses> statusesLiveData = null;
        switch (timelineType) {
            case HOME:
                statusesLiveData = misskeyTimelinesVM.getHomeTimeline(instance, token, maxId, sinceId, limit);
                break;
            case LOCAL:
                statusesLiveData = misskeyTimelinesVM.getLocalTimeline(instance, token, maxId, sinceId, limit);
                break;
            case PUBLIC:
                statusesLiveData = misskeyTimelinesVM.getGlobalTimeline(instance, token, maxId, sinceId, limit);
                break;
            case TAG:
            case ART:
                if (search != null) {
                    String hashtag = search.startsWith("#") ? search.substring(1) : search;
                    statusesLiveData = misskeyTimelinesVM.getHashtagTimeline(instance, token, hashtag, maxId, sinceId, limit);
                }
                break;
            case LIST:
                if (list_id != null) {
                    statusesLiveData = misskeyTimelinesVM.getListTimeline(instance, token, list_id, maxId, sinceId, limit);
                }
                break;
            case ACCOUNT_TIMELINE:
                if (accountTimeline != null) {
                    statusesLiveData = misskeyTimelinesVM.getUserNotes(instance, token, accountTimeline.id, maxId, sinceId, limit, !exclude_replies, media_only);
                }
                break;
        }

        if (statusesLiveData != null) {
            if (direction == null) {
                String finalMaxId = maxId;
                statusesLiveData.observe(getViewLifecycleOwner(), statuses -> {
                    if (finalMaxId != null && timelineType == Timeline.TimeLineEnum.HOME
                            && (statuses == null || statuses.statuses == null || statuses.statuses.isEmpty())) {
                        max_id = null;
                        routeMisskey(null, false, null);
                        return;
                    }
                ...
                statusesLiveData.observe(getViewLifecycleOwner(), statusesBottom -> dealWithPagination(statusesBottom, DIRECTION.BOTTOM, fetchingMissing, true, fetchStatus));
            } else if (direction == DIRECTION.TOP) {
                statusesLiveData.observe(getViewLifecycleOwner(), statusesTop -> dealWithPagination(statusesTop, DIRECTION.TOP, fetchingMissing, true, fetchStatus));
            } else if (direction == DIRECTION.REFRESH || direction == DIRECTION.SCROLL_TOP || direction == DIRECTION.FETCH_NEW) {
                statusesLiveData.observe(getViewLifecycleOwner(), statusesRefresh -> {
                    if (statusAdapter != null) {
    ...
    private void logTimelineLoad(DIRECTION direction, String source, TimelinesVM.TimelineParams timelineParams, Statuses statuses, boolean fetchingMissing, int displayedBefore) {
        if (timelineType != Timeline.TimeLineEnum.HOME || getActivity() == null) {
            return;
        }
        ...
        loadLogs.user_id = account.user_id;
        loadLogs.instance = account.instance;
        loadLogs.slug = Timeline.TimeLineEnum.HOME.getValue();
        loadLogs.direction = direction != null ? direction.name() : TimelineLoadLogs.DIRECTION_INITIAL;
        loadLogs.source = source;
        loadLogs.max_id = timelineParams != null ? timelineParams.maxId : null;
        loadLogs.min_id = timelineParams != null ? timelineParams.minId : null;
        loadLogs.returned = statuses != null && statuses.statuses != null ? statuses.statuses.size() : 0;
        loadLogs.displayed = displayedBefore;
        loadLogs.added = (timelineStatuses != null ? timelineStatuses.size() : 0) - displayedBefore;
        loadLogs.fetching_missing = fetchingMissing;
        loadLogs.trigger = timelineParams != null ? timelineParams.trigger : null;
        Context context = requireActivity().getApplicationContext();
        new Thread(() -> {
            try {
                new TimelineLoadLogs(context).insert(loadLogs);
            } catch (DBException e) {
                e.printStackTrace();
            }
        }).start();
        ...
        if (direction == null) {
            timelinesVM.getTimelineCache(timelineStatuses, timelineParams)
                    .observe(getViewLifecycleOwner(), statusesCached -> {
                        ...
                        logTimelineLoad(null, TimelineLoadLogs.SOURCE_CACHE, timelineParams, statusesCached, fetchingMissing, displayedBefore);
                    });
        } else if (direction == DIRECTION.BOTTOM) {
            timelinesVM.getTimelineCache(timelineStatuses, timelineParams)
            ...
            timelinesVM.getTimelineCache(timelineStatuses, timelineParams)
                    .observe(getViewLifecycleOwner(), statusesRefresh -> {
                        ...
                        logTimelineLoad(direction, TimelineLoadLogs.SOURCE_CACHE, timelineParams, statusesRefresh, fetchingMissing, displayedBefore);
                    });
        }
    }
        ...
        if (direction == null) {
            timelinesVM.getTimeline(timelineStatuses, timelineParams)
                    .observe(getViewLifecycleOwner(), statuses -> {
                        ...
                        logTimelineLoad(null, TimelineLoadLogs.SOURCE_API, timelineParams, statuses, fetchingMissing, displayedBefore);
                    });
        } else if (direction == DIRECTION.BOTTOM) {
            timelinesVM.getTimeline(timelineStatuses, timelineParams)
                    .observe(getViewLifecycleOwner(), statusesBottom -> {
                        int displayedBefore = timelineStatuses != null ? timelineStatuses.size() : 0;
                        dealWithPagination(statusesBottom, DIRECTION.BOTTOM, fetchingMissing, canScroll, fetchStatus);
                        logTimelineLoad(DIRECTION.BOTTOM, TimelineLoadLogs.SOURCE_API, timelineParams, statusesBottom, fetchingMissing, displayedBefore);
                    });
        } else if (direction == DIRECTION.TOP) {
            timelinesVM.getTimeline(timelineStatuses, timelineParams)
                    .observe(getViewLifecycleOwner(), statusesTop -> {
                        int displayedBefore = timelineStatuses != null ? timelineStatuses.size() : 0;
                        dealWithPagination(statusesTop, DIRECTION.TOP, fetchingMissing, canScroll, fetchStatus);
                        logTimelineLoad(DIRECTION.TOP, TimelineLoadLogs.SOURCE_API, timelineParams, statusesTop, fetchingMissing, displayedBefore);
                    });
        } else if (direction == DIRECTION.REFRESH || direction == DIRECTION.SCROLL_TOP || direction == DIRECTION.FETCH_NEW) {
            timelinesVM.getTimeline(timelineStatuses, timelineParams)
                    .observe(getViewLifecycleOwner(), statusesRefresh -> {
                        ...
                        logTimelineLoad(direction, TimelineLoadLogs.SOURCE_API, timelineParams, statusesRefresh, fetchingMissing, displayedBefore);
                    });
        }
    }
    ...
    private void route(DIRECTION direction, boolean fetchingMissing, Status fetchStatus) {
        ...
        app.fedilab.android.mastodon.client.entities.app.Account.API routeApi = BaseMainActivity.api;
        if (routeApi == null && Helper.getCurrentAccount(requireActivity()) != null) {
            routeApi = Helper.getCurrentAccount(requireActivity()).api;
        }
        boolean isMisskey = routeApi == app.fedilab.android.mastodon.client.entities.app.Account.API.MISSKEY;
        // --- HOME TIMELINE ---
        if (timelineType == Timeline.TimeLineEnum.HOME) {
            ...
            app.fedilab.android.mastodon.client.entities.app.Account.API profileApi = BaseMainActivity.api;
            if (profileApi == null && Helper.getCurrentAccount(requireActivity()) != null) {
                profileApi = Helper.getCurrentAccount(requireActivity()).api;
            }
            if (profileApi == app.fedilab.android.mastodon.client.entities.app.Account.API.MISSKEY && !checkRemotely) {
                routeMisskey(direction, fetchingMissing, fetchStatus);
                return;
            }
            ...
            app.fedilab.android.mastodon.client.entities.app.Account.API searchApi = BaseMainActivity.api;
            if (searchApi == null && Helper.getCurrentAccount(requireActivity()) != null) {
                searchApi = Helper.getCurrentAccount(requireActivity()).api;
            }
            if (searchApi == app.fedilab.android.mastodon.client.entities.app.Account.API.MISSKEY) {
                MisskeyTimelinesVM misskeySearchVM = new ViewModelProvider(FragmentMastodonTimeline.this).get(viewModelKey, MisskeyTimelinesVM.class);
                String maxIdSearch = null;
                if (direction == DIRECTION.BOTTOM) {
                    maxIdSearch = max_id;
                }
                if (direction == null) {
                    misskeySearchVM.searchNotes(BaseMainActivity.currentInstance, BaseMainActivity.currentToken, search.trim(), null, null, MastodonHelper.statusesPerCall(requireActivity()))
                            .observe(getViewLifecycleOwner(), this::initializeStatusesCommonView);
                } else if (direction == DIRECTION.BOTTOM) {
                    misskeySearchVM.searchNotes(BaseMainActivity.currentInstance, BaseMainActivity.currentToken, search.trim(), maxIdSearch, null, MastodonHelper.statusesPerCall(requireActivity()))
                            .observe(getViewLifecycleOwner(), statusesBottom -> dealWithPagination(statusesBottom, direction, false, true, fetchStatus));
                } else {
                    flagLoading = false;
                }
            } else {
            ...
            if (isMisskey) {
                ...
                if (reverseOrder) {
                    if (direction == null || direction == DIRECTION.REFRESH) {
    ...
    public void onClickMinId(String min_id, Status fetchStatus) {
        ...
        loadTrigger = "button_newer" + markerSide(fetchStatus);
    ...
    public void onClickMaxId(String max_id, Status fetchStatus) {
        loadTrigger = "button_older" + markerSide(fetchStatus);
    ...
    public void autoFetch(String min_id, String max_id, Status fetchStatus) {
        ...
        loadTrigger = (fetchNewer ? "auto_newer" : "auto_older") + (isUserDragging ? "_drag" : "_idle") + markerSide(fetchStatus);
        if (fetchNewer) {
            min_id_fetch_more = min_id;
            route(DIRECTION.TOP, true, fetchStatus);
        } else {
    ...
    private String markerSide(Status fetchStatus) {
        if (fetchStatus == null || fetchStatus.positionFetchMore == null) {
            return "";
        }
        return "_" + fetchStatus.positionFetchMore.name();
    }
